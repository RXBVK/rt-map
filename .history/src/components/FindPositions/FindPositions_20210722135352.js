...
export default function FindPositions(props) {
  useEffect(() => {
    ...
    //     });
    // }
    props.dataFromRows.forEach((address) => {
      axios
        .get("https://maps.googleapis.com/maps/api/geocode/json", {
          params: {
            address: `${address.address}${address.city}`,
        ...
        .then((response) => {
          if (response.data.results.length == 0) {
            ...
            console.log("Location: ");
            console.log(location);
            let addressWithPosition = { ...address, position: location };
            props.setDataWithCoords([
              ...props.dataWithCoords,
              addressWithPosition,
            ]);
            console.log("Address: ");
            console.log(addressWithPosition);
          }
        });
    });
  }, []);
