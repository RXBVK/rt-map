...
export default function ColumnsPicker(props) {
  ...
  const connectColumnsToData = async () => {
    ...
    let addPosition = properData.map((row) => {
      let positionToAdd = await getPosition(row);
      return { ...row, position: positionToAdd };
    });
    //let temp = await getPosition(properData[2]);
