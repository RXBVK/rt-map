...
function Mapv({ dataFromRows }) {
  ...
  const getPosition = async (address) => {
    ...
    try {
      ...
      if (response.data.results.length > 0) {
        let location = response.data.result[0].geometry.location;
        result = location;
      } else {
        console.log("Something went wrong, check address");
      }
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <>
      ...
      <button onClick={() => console.log(getPosition(dataFromRows[2]))}>
        TESTS
      </button>
    </>
  );
